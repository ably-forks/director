...
Router.prototype.on = function (method, path) {
  ...
  if (options) {
    ...
    if (options.metadata) {
      /* Bit hacky to add it as a property of the route, but convenient */
      route._metadata = options.metadata;
    }
  }
...
Router.prototype.dispatch = function (req, res, callback) {
  ...
  // Don't decodeURI here: the individual components end up partially decoded
  // (as decodeURI doesn't touch anything that can have a special meaning in a
  // url, ie any of ; , / ? : @ & = + $). Instead call decodeURIComponent
  // application-side on each individual routeParam
